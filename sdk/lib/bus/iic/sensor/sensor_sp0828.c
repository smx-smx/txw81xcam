...
/*
 * NOTE ON DETECTION (TEMPORARY):
 * The board returns 0xFF for all reads until MCLK/RESET are active.
 * To let the pipeline start and enable clocks, we temporarily detect
 * by expecting 0xFF at page0/reg0x00. After video comes up, use the
 * CLI dumper to find the true ID and replace the ident tuple.
 *
 * FINAL (likely):  id=0x0C, id_reg=0x02
 * TEMPORARY:       id=0xFF, id_reg=0x00   (address-only-ish)
 */
...
SENSOR_INIT_SECTION static const unsigned char SP0828InitTable[CMOS_INIT_LEN] =
{
    ...
    0	f4,0x1b, 0xf5,0x97,
    0	ec,0x53, 0xed,0x78, 0xee,0x47, 0xef,0x6c,
    0	f7,0x70, 0xf8,0x5b, 0xf9,0x64, 0xfa,0x4f,
    ...
    0xc2,0x41, 0xc3,0x41, 0xc4,0x41, 0xc5,0x70, 0xc6,0x41, 0xca,0x70, 0	cb,0x0c,
...
SENSOR_OP_SECTION const _Sensor_Adpt_ sp0828_cmd =
{
    ...
    .p_fun_adapt = { NULL, NULL, NULL },   // rotate / hvblank / exp-gain (none custom)
    .p_xc7016_adapt = {NULL},
};

/* TEMPORARY address-only-ish detection:
 * Expect 0xFF @ reg 0x00 (page 0) using 0x30/0x31 (8-bit addr/data).
 * Replace with real ID once MCLK is up and peek shows true bytes, e.g.:
